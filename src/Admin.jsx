...
function Admin() {
  ...
  const handleAddBlock = (e) => {
    ...
    .catch(err => {
        ...
        alert("Nem sikerült elmenteni a blokkolást!");
    });
  };
  ...
  return (
    <div style={{ backgroundColor: '#f4f6f8', minHeight: '100vh', paddingBottom: '50px' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '30px auto', padding: '0 15px' }}>
        ...
        {/* NAPTÁR FEJLÉC ÉS GOMBOK - Reszponzív (flex-wrap) */}
        ...
        {/* A NAPTÁR RÁCS - Mobilon görgethető (overflowX: 'auto') */}
                ...
                {weekDays.map(day => {
                    ...
                    <div key={day.toString()} style={{ flex: 1, borderRight: '1px solid #e0e0e0', position: 'relative' }}>
                        ...
                        {blocks.filter(b => isSameDay(new Date(b.start_time), day)).map(block => {
                            ...
                            return (
                                <div key={`block-${block.id}`} onClick={() => handleDeleteBlock(block.id)}
                                    style={{ 
                                        position: 'absolute', top: `${topPosition}px`, left: '2px', right: '2px', height: `${height}px`, 
                                        background: 'repeating-linear-gradient(45deg, #e0e0e0, #e0e0e0 10px, #cccccc 10px, #cccccc 20px)',
                                        borderRadius: '4px', cursor: 'pointer', zIndex: 5, border: '1px solid #bbb',
                                        display: 'flex', justifyContent: 'center', alignItems: 'center', opacity: 0.8
                                    }} title="Kattints a törléshez"
                                >
                                    <span style={{ fontWeight: 'bold', fontSize: '0.75rem', backgroundColor: 'rgba(255,255,255,0.9)', padding: '2px 5px', borderRadius: '4px', textAlign: 'center' }}>
                                        🚫 Törlés
                                    </span>
                                </div>
                            );
                        ...
                        {appointments.filter(a => isSameDay(new Date(a.start_time), day)).map(appt => {
                            ...
                            // Itt dől el, hogy rövid-e a masszázs
                            ...
                            return (
                                <div key={appt.id} onClick={() => setSelectedAppt(appt)}
                                    style={{ 
                                        position: 'absolute', top: `${topPosition}px`, left: '2px', right: '2px', height: `${height}px`, 
                                        backgroundColor: selectedAppt?.id === appt.id ? '#f39c12' : (appt.source === 'admin' ? '#3498db' : '#27ae60'), 
                                        color: 'white', borderRadius: '4px', 
                                        padding: '4px 6px', cursor: 'pointer', zIndex: 10, border: '1px solid rgba(0,0,0,0.1)', 
                                        
                                        // --- ITT VAN A VARÁZSLAT ---
                                        display: 'flex', 
                                        flexDirection: isShort ? 'row' : 'column', // Ha rövid, egymás mellé teszi
                                        alignItems: isShort ? 'center' : 'flex-start', // Rövidnél középre igazítja a sort
                                        gap: isShort ? '8px' : '2px', // Pici szóköz az idő és a név közé
                                        overflow: 'hidden'
                                    }}
                                >
                                    {/* Időpont (A flexShrink: 0 miatt sosem nyomódik össze) */}
                                    <div style={{ fontWeight: 'bold', fontSize: '0.75rem', opacity: 0.9, flexShrink: 0 }}>
                                        {format(start, 'HH:mm')}
                                    </div>
                                    
                                    {/* Név */}
                                    <div style={{ fontWeight: 'bold', fontSize: '0.75rem', textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden' }}>
                                        {appt.customer_name}
                                    </div>
                                </div>
                            );
        ...
        {/* CMS TÁBLÁZAT - Görgethető mobilon */}
